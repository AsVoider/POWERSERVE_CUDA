...
def get_output_folder(folder, batch_size, htp_version):
    ...
    run_shell_command(
        f"cp {htp_lib_folder}/libQnnHtpV{htp_version}.so {folder}"
    )
    run_shell_command(
        f"cp {htp_lib_folder}/libQnnHtpV{htp_version}Skel.so {folder}"
    )

    if htp_version == 79:
        run_shell_command(
            f"cp {htp_lib_folder}/libQnnHexagonSkel_dspApp.so {folder}"
        )
