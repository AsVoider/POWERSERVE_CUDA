...
class LlamaAttentionCore(nn.Module):
    def __init__(self, layer_id: int, n_kv_heads: int, group_size: int):
        super().__init__()
    ...
    def forward(
        ...
        for i in range(self.n_kv_heads):
            ...
            all_keys_t = torch.cat((key_t_caches[i], scaled_key.transpose(0, 1)), dim=-1)
            all_values = torch.cat((value_caches[i], scaled_value), dim=-2)
...
class LlamaAttention(nn.Module):
    def __init__(
        ...
        self.rope = LlamaRoPE()
        self.core = LlamaAttentionCore(layer_id=layer_id, n_kv_heads=n_kv_heads, group_size=self.group_size)
    ...
    def forward(
        ...
        for key, monitor in zip(keys, self.key_monitors, strict=True):
            monitor.track_norm(key)
        for value, monitor in zip(values, self.value_monitors, strict=True):
...
class KVCache:
    ...
    def update_kv_caches(self, kv: Tuple[torch.Tensor]):
        ...
        for cache, key in zip(self.key_t_caches, keys, strict=True):
            cache[:, beg:end] = key.transpose(0, 1)
        for cache, value in zip(self.value_caches, values, strict=True):
            cache[beg:end, :] = value
...
class LlamaModel(nn.Module):
    ...
    def get_attention_bias(self, batch_size: int) -> torch.Tensor:
        ...
        attn_bias = torch.full(
            size=(batch_size, self.graph_params.cache_size + batch_size),
        ...
        # Causal mask
        attn_bias[:, -batch_size:] = torch.full(
    ...
    def eval_batch(
        ...
        if save_samples:
            self.input_embedding.saved_samples.append(Sample((input_ids,), (x,)))
...
# export the input and output embedding
print("Exporting input embedding...")
output_folder = args.output_folder / "input_embedding" / args.graph_name
output_folder.mkdir(parents=True, exist_ok=True)

exporter = OutputEmbeddingExporter(
    graph_name=args.graph_name,
    model_chunk=model.input_embedding,
