...
struct KVCache {
	...
	KVCache(std::shared_ptr<LlamaConfig> &config, size_t seq_len, size_t kv_dim)
		: config(config),
		  seq_len(seq_len),
		  kv_dim(kv_dim),
		  key_cache(DataType::FP32, {config->dim * config->n_kv_heads / config->n_heads, config->seq_len, config->n_layers}),
		  value_cache(DataType::FP32, {config->dim * config->n_kv_heads / config->n_heads, config->seq_len, config->n_layers}) {
