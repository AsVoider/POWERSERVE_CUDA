...
auto Qwen2Model::forward(
...
#endif
    {
        if (!lazy_load) {
            // TODO: fuck CPU
            static_cast<ggml::GGMLBackend &>(*m_platform->backends[m_config->model_id][TensorBackend::GGML_CPU])
                .reset_kv_batch_size(batch_size);
            for (size_t L = 0; L < llm_config.n_layers; L++) {
                auto [k_cache, v_cache] =
                    static_cast<ggml::GGMLBackend &>(*m_platform->backends[m_config->model_id][TensorBackend::GGML_CPU])
                        .m_kv->get_cache(L);
...
#endif
    {
        // TODO: save GPU
        static_cast<ggml::GGMLBackend &>(*m_platform->backends[m_config->model_id][TensorBackend::GGML_CPU])
            .m_kv->advanced_kv_cache_size(batch_size);
    }
