...
void Executor::allocate_buffers() {
	for (auto tensor : graph_.tensors) {
		...
		case DataType::FP32: {
			tensor->data_ = platform_.ggml_backend.create_buffer<float>(tensor->shape_);
		} break;

		case DataType::INT32: {
			tensor->data_ = platform_.ggml_backend.create_buffer<int32_t>(tensor->shape_);
		} break;
...
void Executor::run() {
	for (auto op : graph_.ops) {
		...
		case OpType::MAT_MUL: {
			...
			auto out	= op->output();
			platform_.ggml_backend.matmul(out, weight, x);
		} break;
		...
		case OpType::RMS_NORM: {
			...
			auto weight = op->prev[1]->tensor();
			auto out	= op->output();
			platform_.ggml_backend.rmsnorm(out, x, weight);
		} break;
		...
		case OpType::SILU_HADAMARD: {
			...
			auto out  = op->output();
			platform_.ggml_backend.silu_hadamard(out, gate, up);
		} break;
		...
		case OpType::ROPE: {
			...
			platform_.ggml_backend.rope(q_out, k_out, q, k, pos);
		} break;

		case OpType::SOFTMAX: {
			auto x	 = op->prev[0]->tensor();
			auto out = op->output();
			platform_.ggml_backend.softmax(out, x);
		} break;
		...
		case OpType::MHA: {
			...
			platform_.ggml_backend.multihead_attention(out, q, key_cache, val_cache, pos, layer_id);
		} break;
		...
		case OpType::COPY: {
			...
			platform_.ggml_backend.copy(dst, src, off);
		} break;
